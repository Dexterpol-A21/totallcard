...
// Quitar la línea del 1 (hexágono abierto)
content = content.replace(/<svg class="absolute inset-0 w-full h-full text-corporate-red" viewBox="0 0 100 100" fill="none" stroke="currentColor" stroke-width="2\.5">\s*<path d="M 50 15 L 15 30 L 15 65 L 30 80" stroke-linejoin="round" stroke-linecap="round"\/>\s*<\/svg>/g, '');

const oldCards = /<!-- Elementos Gráficos Derecha \(Tarjetas Muestra\) -->[\s\S]*?(?=<\/div>\s*<\/div>\s*<!-- Fin Paso 1|<!-- Paso 2|$)/;

const newCards = `<!-- Elementos Gráficos Derecha (Tarjetas Muestra) -->
                    <div class="shrink-0 hidden lg:flex gap-6 xl:gap-10 items-center pt-2 relative">
                        <!-- Card 1: Frente -->
                        <div class="relative flex flex-col items-center">
                            
                            <!-- Tarjeta Estructura Mokeada grande -->
                            <div class="w-[280px] h-[175px] bg-white rounded-[10px] shadow-[0_6px_20px_rgba(0,0,0,0.12)] border border-slate-50 relative overflow-hidden flex flex-col p-4.5 p-4 z-10">
                                
                                <div class="text-[0.65rem] font-bold text-center mt-3 mb-3 tracking-wider text-black flex items-center justify-center gap-1.5">
                                    TOTALLCARD 
                                ...
                                <div class="flex gap-4 items-center px-2">
                                    ...
                                    <div class="pt-1.5">
                                        <div class="text-[0.7rem] font-bold text-black leading-none mb-1">ANDRÉS RAMÍREZ</div>
                                ...
                                <div class="mt-auto w-full flex flex-col items-center mb-1">
                                    <div class="h-6 w-[80%] bg-black/80 masked-barcode" style="-webkit-mask-image: repeating-linear-gradient(90deg, #000 0px, #000 3px, transparent 3px, transparent 5px);"></div>
                                    <div class="text-[0.5rem] text-slate-500 tracking-[0.25em] mt-1.5">EMP-001</div>
                                </div>

                                <!-- Decoración top left y mark -->
                                <div class="absolute top-0 left-0 w-full h-full pointer-events-none">
                                    <svg viewBox="0 0 280 175" class="w-full h-full">
                                        <path d="M 0 0 L 35 0 L 15 20 Z" fill="#e81d2c" />
                                        <path d="M -10 -5 L 60 30" stroke="#e81d2c" stroke-width="2" />
                                    </svg>
                                </div>
                        ...
                        <div class="relative flex flex-col items-center">
                            ...
                            <div class="w-[280px] h-[175px] bg-white rounded-[10px] shadow-[0_6px_20px_rgba(0,0,0,0.12)] border border-slate-50 relative overflow-hidden flex items-center p-4 z-10">
...
content = content.replace(/<!-- Elementos Gráficos Derecha \(Tarjetas Muestra\) -->([\s\S]*?)<\/div>\s*<\/div>\s*<\/div>(\s*<!-- FAQ \/ Preguntas)/, newCards + '\n                </div>\n            </div>$2');

fs.writeFileSync('src/pages/faq.astro', content);
console.log('Cards resized and line removed');
